...
public static void main(String[] args)  throws IOException
{
	...
	printReads(input, coords, readOut);
	...
	for(int i = 0; i<n; i++)
	{
		...
		for(int j = Math.max(0, i - radius); j <= i + radius && j < n; j++)
		{
			...
			if(coords.get(j).a <= coords.get(i).a && coords.get(j).b >= coords.get(i).b)
			{
				...
				maxContainmentScore = Math.max(maxContainmentScore, score(coords.get(i).a, coords.get(i).b, coords.get(j).a, coords.get(j).b));
			}
			else
			{
				// Not contained so update max overhang
				minOverhang = Math.min(minOverhang, (int)(Math.max(0, coords.get(j).a - coords.get(i).a) + Math.max(0, coords.get(i).b - coords.get(j).b)));
			}
		}
...
static void printReads(Scanner input, ArrayList<Pair> coords, PrintWriter out)
{
	int bufferLength = 50000;
	Collections.sort(coords, new Comparator<Pair>() {
		public int compare(Pair a, Pair b) {
			
			if(a.b != b.b) Long.compare(a.b, b.b);
