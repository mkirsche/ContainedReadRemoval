...
if [ "$assembler" = "canu" ]; then
    ...
    $canufile -d $OUTDIR -p $outfile -genomeSize=$length -stopOnLowCoverage=1 -useGrid=true gridOptions="--partition shared --time 3-0:0:0 --mem-per-cpu=4900MB --mem=117600MB" -$readtype $readfile
else
